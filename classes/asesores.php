...
class asesores extends db{
  ...
  public function getAll($orderby = 'a.usuarioid', $order = 'DESC', $limit = 20, $offset = 0, $search = false){
    ...
    $q = 'SELECT a.* FROM usuarios_representantes as a WHERE 0 < usuarioid ';
    if(count($search)){
      $sstr = '';
      foreach($search as $key=>$field){
        $sstr .= ' AND '.$key.' LIKE "%'.$field.'%"';
    ...
    foreach($this->result as $key=>$value){
      $q = 'SELECT * FROM relacionconvrepre WHERE idRepresentante='.$value['usuarioid'];
      $convs = $this->assoc($q);
      foreach($convs as $conv){
        $this->result[$key]['conv_'.$conv['idConvocatoria']] = true;
      }
    }
  ...
  public function getAdmins($orderby = 'a.usuarioid', $order = 'DESC', $limit = 20, $offset = 0, $search = false){
    //$q = 'SELECT a.* FROM usuarios_representantes as a WHERE usuarioid NOT IN (SELECT id_asesor FROM visita GROUP BY id_asesor) ORDER BY usuarioid DESC';
    $q = 'SELECT a.* FROM usuarios as a WHERE 0 < usuarioid ';
    if(count($search)){
      $sstr = '';
      foreach($search as $key=>$field){
        $sstr .= ' AND '.$key.' LIKE "%'.$field.'%"';
